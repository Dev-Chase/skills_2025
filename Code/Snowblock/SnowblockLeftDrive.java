...
public class SnowblockEncoder extends LinearOpMode {
  ...
  private static final double ARM_TUNE_TICK_DELTA = TICKS_PER_DEGREE;
  private static final double ARM_TUNE_THRESHOLD = 0.35;
  ...
  public void runOpMode() {
    ...
    if (opModeIsActive()) {
      while (opModeIsActive()) {
        ...
        // Manually Tuning Arm Position
        if (gamepad1.y && Math.abs(gamepad1.left_stick_y) > ARM_TUNE_THRESHOLD && arm_timer.milliseconds() >= ARM_TUNE_DELAY) {
          arm_target_pos += ARM_TUNE_TICK_DELTA;
        }
